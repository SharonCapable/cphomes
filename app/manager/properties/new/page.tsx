...
import { createProperty } from '@/lib/actions/manager'
...
export default function NewPropertyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Process images: If it's a file, we could upload it or for now just use the blob/placeholder
      // In a real app, you'd upload to S3/Supabase here.
      // For now, let's convert to Base64 so they actually save string data that can be viewed
          ...
          return new Promise<{ url: string; isPrimary: boolean }>((resolve) => {
            ...
            reader.onloadend = () => {
              ...
              imgElement.onload = () => {
                ...
                resolve({ url: compressedDataUrl, isPrimary: img.isPrimary })
              }
      ...
      if (result.success) {
        toast.success("Estate successfully curated into the collection.")
        router.push('/manager/properties')
      } else {
        toast.error(result.error || "A disruption occurred during curation.")
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-bold heading-serif italic">Create New Listing</h1>
          <p className="text-sm text-muted-foreground font-light uppercase tracking-widest mt-1">Directly add a property to the ecosystem</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          >
            {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Confirm & Publish Listing'}
          </Button>
        </div>
      </form>
    </div>
